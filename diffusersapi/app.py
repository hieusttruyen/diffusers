...
import PIL
import json
from diffusersapi.loadModel import loadModel
from diffusersapi.send import send, getTimings, clearSession
from .status import status
import os
import numpy as np
import skimage
import skimage.measure
from diffusersapi.getScheduler import getScheduler, SCHEDULERS
from diffusersapi.getPipeline import getPipelineForModel, listAvailablePipelines, clearPipelines
import re
import requests
from diffusersapi.download import download_model, normalize_model_id
import traceback
...
HF_AUTH_TOKEN = "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"
...
async def inference(all_inputs: dict, response) -> dict:
    ...
    if MODEL_ID == "ALL":
        if last_model_id != normalized_model_id:
            clearPipelines()
            model = loadModel(MODEL_ID, send_opts=send_opts)
            last_model_id = normalized_model_id
    else:
